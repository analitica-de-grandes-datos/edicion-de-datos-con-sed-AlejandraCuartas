...
#
sed 's/\/\([0-9][0-9]\);/\/20\1;/' data.csv | sed 's/\([0-9]\/[0-9]\/\)/0\1/' | sed 's/\/\([0-9]\)\//\/0\1\//' | sed 's/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9][0-9][0-9]\)/\3-\2-\1/g' | sed 's/;\r/;\\N/' | sed 's/;;/;\\N;/' | sed 's/[a-z]/\U&/g'  | sed 's/;N/;\\N/'  | sed 's/,/./' | sed 's/;/,/g'